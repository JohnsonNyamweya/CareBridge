...
const RelatedDoctors = ({docId, speciality}) => {
  ...
  return (
    <div className='flex flex-col items-center gap-4 my-16 text-bg-gray-900 md:mx-10'>
        ...
        <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
            {
                relatedDocs.slice(0,5).map((doctor, index) => (
                    <div onClick={() => {navigate(`/appointment/${doctor._id}`); scrollTo(0,0);}} key={index} 
                    className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer 
                    hover:translate-y-[-10px] transition-all duration-500'>
                        <img className='bg-blue-50' src={doctor.image} alt={`${doctor.image} image`} />
                        <div className='p-4'>
