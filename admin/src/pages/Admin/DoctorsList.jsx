...
const DoctorsList = () => {
  ...
  const {adminToken, doctors, fetchAllDoctors, changeAvailability} = useContext(AdminContext);
      ...
      <div className='w-full flex flex-wrap gap-4 pt-5 gap-y-6'>
        {
          doctors && doctors.map((doctor, index) => (
            <div key={index} className='border border-indigo-200 rounded-xl max-w-56 overflow-hidden cursor-pointer group'>
              <img className='bg-indigo-50 group-hover:bg-primary transition-all duration-500' src={doctor.image} alt={`${doctor.speciality} image`} />
              <div className='p-4'>
